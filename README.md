...
### 1. TechDocs (멀티 에이전트 RAG 기반 특허 문서 AI 검색 서비스)
> 사용자의 질문 의도를 다각도로 분석하여 특허 문서를 검색하고, 신뢰성 있는 답변을 조율하는 멀티 에이전트 기반 RAG 시스템입니다.
* **Stack**: `FastAPI`, `LangGraph`, `LangChain`, `Pinecone`, `SSE (Server-Sent Events)`, `OpenAI`, `RAGAS`, `Docker`
* **Key Achievements**:
  * **멀티 에이전트 워크플로우 설계**: 복잡한 특허 분석을 처리하기 위해 질문 분석, 하이브리드 검색, 답변 검증을 전문적으로 분담하는 **멀티 에이전트 아키텍처(LangGraph)** 구현.
  * **SSE (Server-Sent Events) 기반 스트리밍**: 비결정적인 에이전트들의 실시간 추론 상태(Thought Process), 검색 진행률, 그리고 최종 답변 데이터를 사용자 인터페이스에 부드럽게 전달하기 위해 **FastAPI & React 간 SSE 스트리밍 파이프라인** 구축.
